  ...
  private buildCollectibleMesh() {
    switch (this.type) {
      case 'ORB': {
        ...
        const haloGeo = new THREE.TorusGeometry(0.55, 0.04, 8, 16);
      ...
      case 'MULTIPLIER': {
        ...
        const ringGeo = new THREE.TorusGeometry(0.48, 0.04, 8, 16);
        const ringMat = new THREE.MeshBasicMaterial({ color: 0xff66cc });
        const ring = new THREE.Mesh(ringGeo, ringMat);
        ring.rotation.x = Math.PI / 2;

        this.mesh.add(bolt1, bolt2, ring);
        break;
      }
    }
  }
